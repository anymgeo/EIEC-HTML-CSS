...
$.fn.mainSlider = function(options) {
	...
	var _this = $(this)[0];
	// _this.settings = null;
	// _this.mainSlider_interval = null;
	...
	if(!_this.settings) {
		_this.settings = $.extend({
			...
			gestures:			true,
			theme:				'dark',
			prev_next_nav: 		'outside', 			// options: 'inside', 'outside', 'hover', 'none'
			dots_nav: 			'outside', 			// options: 'inside', 'outside', 'hover', 'none'
	...
	// Handle slideshow argument commands (eg. $('.slideshow').mainSlider('play');)
	if(options == 'play') {
		run_slideshow();
		return;
	}
	...
	$this.addClass('mainSlider loading');
	...
	$($this).append('<div class="mainSlider_caption"></div>');

	// Add slideshow navigation controls (if there is more than 1 image)
	if($($this).find('.image_wrapper').length > 1) $($this).append('<div class="mainSlider_controls"><span class="mainSlider_prev_image">Previous</span><span class="mainSlider_next_image">Next</span><div class="mainSlider_dots"></div></div>');

	// Add loading
	$($this).append('<div class="mainSlider_spinner"></div>');

	// For each img, add dot to dot nav
	var x = 0;
	$('.image_wrapper', $this).each(function() {
		$('.mainSlider_dots', $this).append('<span data-image-num="' + x + '"></span>');
		x++;
	});
	$('.mainSlider_dots span:first-child', $this).addClass('current')
	...
	// Events
	$('.mainSlider_prev_image', $this).click(function() {
		prev_image();
	});

	$('.mainSlider_next_image', $this).click(function() {
		next_image();
	});

	$('.mainSlider_dots span', $this).click(function() {
	...
	function loadImage($wrapper) {
		...
		img.onload = function() {
			...
			else if($wrapper.closest('.mainSlider').find('.image_wrapper').length) {
				load_image_timeout = setTimeout(function() {
					loadImage($wrapper.closest('.mainSlider').find('.image_wrapper').first());
				}, 1000);
			}

			$wrapper.addClass('image_loaded')

			if($wrapper.closest('.mainSlider').find('.image_wrapper:not(.image_loaded)').length < 1) {
	...
	function stop_slideshow() {
		clearInterval(_this.mainSlider_interval);
		_this.mainSlider_interval = null;
	...
	function reset_interval() {
		if(!_this.mainSlider_interval) return;
		clearInterval(_this.mainSlider_interval);
		clearInterval(_this.mainSlider_interval);
		_this.mainSlider_interval = setInterval(next_image, _this.settings['slide_duration'] + _this.settings['transition_time']);
		// $($this).data('interval', _this.mainSlider_interval);
	}
	...
	function update_caption() {
		if(_this.settings['caption'] == 'none') {
			$('.mainSlider_caption', $this).remove();
			return;
		}
		$('.mainSlider_caption', $this).fadeOut(_this.settings['transition_time'], function() {
			setTimeout(function() { // Add a delay so that new image can load before getting new caption
				$('.mainSlider_caption', $this).html($('.current_slide img', $this).attr('alt')).fadeIn(_this.settings['transition_time'] - 200);
			}, 10)
		});
	}
	...
	function update_dots_nav() {
		if(_this.settings['dots_nav'] == 'none') {
			$('.mainSlider_dots', $this).remove();
			return;
		}
		$('.mainSlider_dots span', $this).removeClass('current');
		$('.mainSlider_dots span[data-image-num="' + $('.current_slide', $this).index() + '"]', $this).addClass('current');
	}
	return $this;
